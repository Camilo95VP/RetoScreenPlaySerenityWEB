package co.com.sofka.userinterfaces.llenarformularioincompleto;
...
public class LlenarFormularioIncompleto implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        if (DOCUMENTO.isVisibleFor(actor)) {
            actor.attemptsTo(
                    ...
                    WaitUntil.the(MENSAJE_COMPRA_NO_EXITOSA,isEnabled()).forNoMoreThan(Duration.ofSeconds(20))
            );
        }
    }

    public static LlenarFormularioIncompleto llenarFormularioIncompleto(){
        return new LlenarFormularioIncompleto();
